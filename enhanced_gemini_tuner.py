...
class ChatManager:
    ...
    def create_chat(self, model_name: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            model_instance = self.model_manager.client.generative_model(model_to_use)
...
class ModelManager:
    ...
    def delete_model(self, model_name: str) -> bool:
        """
        Deletes a *tuned* model by its full name (ID).

        Args:
            model_name: The full name/ID of the tuned model (e.g., 'tunedModels/abc-123').

        Returns:
            True if deletion was successful or model not found, False otherwise.
        """
        if not self.is_client_ready():
        ...
        try:
            ...
            # Add confirmation step in interactive/GUI modes here
            self.client.delete_tuned_model(name=model_name)
            logger.info(f"Successfully deleted tuned model: {model_name}")
            return True
        except google_exceptions.NotFound:
            ...
            return True # Treat not found as success in deletion context
             ...
             logger.error(f"Permission denied while trying to delete model '{model_name}'. Detail: {e}")
             return False
        except Exception as e:
    ...
    def test_model(self, model_name: str, test_input: str, generation_config_override: Optional[Dict] = None) -> Optional[str]:
        ...
        try:
            ...
            # Use the client's generative_model helper
            model_instance = self.client.generative_model(model_name)
                 ...
                 try:
                      ...
                      if finish_reason:
                           ...
                           for rating in response.prompt_feedback.safety_ratings:
                                if rating.probability != glm.SafetyRating.Probability.NEGLIGIBLE:
